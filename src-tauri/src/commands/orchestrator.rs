...
pub async fn orchestrator_cli(
    ...
    cmd.arg("--print");
    // `bypassPermissions` (NOT the user's "plan" default, which makes the model
    // burn seconds reasoning about whether it may reply) so it answers directly
    // AND can call the user's configured MCP tools (e.g. Atlassian/Jira) without
    // an interactive permission prompt that `-p` would auto-deny. We deliberately
    // do NOT pass `--strict-mcp-config`, so the user's Claude Code MCP servers
    // load and the orchestrator can look things up beyond the board snapshot.
